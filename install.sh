...
# ── optional python libs for the bbcrawl helper tools (all graceful) ────────
# pyyaml     -> lets apirecon.py parse YAML OpenAPI/Swagger specs (JSON always works)
# jsbeautifier -> optional JS beautification for cleaner regex line-mapping
inf "optional python libs (pyyaml, jsbeautifier) — safe to skip if they fail"
for lib in pyyaml jsbeautifier; do
  python3 -c "import ${lib/pyyaml/yaml}" 2>/dev/null && { ok "$lib already present"; continue; }
  pip3 install --user -q "$lib" 2>/dev/null \
    || pip3 install --user -q --break-system-packages "$lib" 2>/dev/null \
    && ok "installed $lib" || wrn "optional $lib not installed (fine — feature degrades gracefully)"
done
