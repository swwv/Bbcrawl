...
${BOLD}STAGES${END}  (defaults on: passive, active, js, secrets, params)
      --subs              subdomain enumeration first (subfinder/assetfinder/crt.sh)
      --arjun             active hidden-parameter discovery (arjun)
      --nuclei            nuclei exposures / misconfig / takeover scan
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --all)         DO_SUBS=1; DO_PASSIVE=1; DO_ACTIVE=1; DO_JS=1; DO_SECRETS=1
                   DO_PARAMS=1; DO_ARJUN=1; DO_NUCLEI=1; DO_GF=1
                   DO_SCREENS=1; DO_DIRS=1; shift;;
...
# where is jsecret.py? (same dir as this script)
...
PASSIVE="$OUTDIR/raw/passive_urls.txt"; : > "$PASSIVE"
...
ACTIVE="$OUTDIR/raw/active_urls.txt"; : > "$ACTIVE"
...
# all path-bearing endpoints (dedup by path)
if have unfurl; then unfurl -u paths < "$ALL" 2>/dev/null | sort -u > "$ENDPOINTS"
else sed -E 's#^https?://[^/]+##' "$ALL" | sort -u > "$ENDPOINTS"; fi
...
ok "endpoints: ${BOLD}$(countf "$ENDPOINTS")${END}   js: ${BOLD}$(countf "$JSLIST")${END}   param-URLs: ${BOLD}$(countf "$PARAMS")${END}"
ok "crawl subdomains: ${BOLD}$(countf "$NEWSUBS")${END}   interesting: ${BOLD}$(countf "$INTERESTING")${END}"

# ══════════════════════ STAGE 7: JS download + analysis ═══════════════════
SECRETS="$OUTDIR/secrets.txt"; : > "$SECRETS"
JS_ENDPOINTS="$OUTDIR/endpoints_from_js.txt"; : > "$JS_ENDPOINTS"
if [[ $DO_JS -eq 1 && -s "$JSLIST" ]]; then
  phase "JS download & analysis"
  # download JS with httpx (or curl fallback)
  info "downloading $(countf "$JSLIST") JS files"
  if have httpx; then
    # store-response saves bodies into a folder; filter to 200s
    httpx -l "$JSLIST" "${HXFLAGS[@]}" -mc 200 -srd "$OUTDIR/js_files" 2>/dev/null >/dev/null || true
  fi
  # curl fallback / top-ups for anything not saved
  if [[ -z "$(ls -A "$OUTDIR/js_files" 2>/dev/null)" ]]; then
    idx=0
    while read -r ju; do
      idx=$((idx+1)); [[ $idx -gt 800 ]] && break
      fn="$OUTDIR/js_files/$(echo "$ju" | md5sum | cut -c1-16).js"
      curl -s --max-time 15 ${INSECURE:+-k} ${PROXY:+-x "$PROXY"} \
           ${COOKIE:+-H "Cookie: $COOKIE"} "$ju" -o "$fn" 2>/dev/null || true
    done < "$JSLIST"
  fi
  jscount=$(find "$OUTDIR/js_files" -type f 2>/dev/null | wc -l | tr -d ' ')
  ...
  if have jsluice; then
    ...
    find "$OUTDIR/js_files" -type f -print0 2>/dev/null | \
  ...
  # jsecret.py: regex secret + endpoint engine (endpoints always; secrets gated)
  if [[ -f "$JSECRET" ]]; then
    info "jsecret.py regex engine over saved JS"
    JSARGS=(-d "$OUTDIR/js_files" --no-color --json "$OUTDIR/raw/jsecret.json"
            --endpoints-out "$JS_ENDPOINTS")
    if [[ $DO_SECRETS -eq 1 ]]; then JSARGS+=(--secrets-out "$SECRETS")
  ...
  if [[ $DO_SECRETS -eq 1 ]] && have trufflehog; then
    ...
    trufflehog filesystem "$OUTDIR/js_files" --no-update --json 2>/dev/null \
...
SUMMARY="$OUTDIR/SUMMARY.txt"
{
  ...
  echo "-------------------------------------------"
  hi=$(grep -icE 'high' "$SECRETS" 2>/dev/null); hi=${hi:-0}
  echo "high-severity secret lines: $hi"
  echo "review first: interesting.txt, secrets.txt, endpoints_from_js.txt"
} | tee "$SUMMARY"
